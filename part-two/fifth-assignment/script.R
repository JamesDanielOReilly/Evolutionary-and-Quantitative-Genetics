...
# YOU CAN ALSO FIND THIS FILE ON MINERVA (FOLDER 6; REMEMBER TO CHANGE THE FILE NAME)
...
# when the correlation between X1, X2 and X3 is very strong, multicolinearity can be an issue. In this case, all
# the correlations are significant, but the correlations are not extremely high. The pairs() function shows that 
# the relationships show quite some scatter. This suggests we will not have big multicolinearity issues here.

# However, we want a statistical confirmation of this,  one that also takes into account how X1, X2 and X3 covary with Y. 
...
# Then search in Google for an R script or package that allows to calculate vif, and try to run it on the data.
...
par(mfrow = c(2,2))
plot(geogenvar$distance,geogenvar$AR)
plot(geogenvar$all_barriers,geogenvar$AR)
plot(geogenvar$log10.habitat.width.,geogenvar$AR)
plot(geogenvar$log10.upstream.distance.,geogenvar$AR)
...
# We conclude that only the effect of barriers on fst is substantial, since this effect remains significant even when we account for distance (first mantel.partial test)
# The effect of distance on fst is not substantail, since this effect is no longer significant when we account for barriers (second mantel.partial test)
...
par(mfrow = c(2,2))
plot(geogendist$distance,geogendist$fst)
plot(geogendist$all_barriers,geogendist$fst)
plot(geogendist$log10.habitat.width.,geogendist$fst)
plot(geogendist$log10.upstream.distance.,geogendist$fst)
